...
def build_task_list_message(tasks: list) -> str:
    if not tasks:
...
def build_task_keyboard(tasks: list) -> InlineKeyboardMarkup:
    keyboard = []
...
def build_priority_keyboard(task_index: int) -> InlineKeyboardMarkup:
    keyboard = [
...
def save_today_to_history(user_id: int):
...
async def finish_collecting(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    user_id = update.effective_user.id
    user_data = get_user_data(user_id)
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("priority_"):
        ...
        if 0 <= task_index < len(user_data["tasks"]):
            ...
            await query.edit_message_text(
                f"{p_emoji} کار *{task_name}* با اولویت *{p_label}* ثبت شد!\n\n"
        ...
        if not history:
            await query.edit_message_text(
                ...
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 برگشت", callback_data="status")]])
            )
            return
        ...
        for day in sorted_days:
            ...
            lines.append(f"📆 {day}")
            lines.append(f"  {bar} {h['percent']}%")
            lines.append(f"  ✅ {h['done']} از {h['total']} کار\n")
        await query.edit_message_text(
            "\n".join(lines),
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 برگشت", callback_data="status")]])
        ...
        for week_key in sorted_weeks:
            ...
            lines.append(f"  ✅ {w['done_tasks']} از {w['total_tasks']} کار در {w['days']} روز\n")
        await query.edit_message_text(
            "\n".join(lines),
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 برگشت", callback_data="status")]])
    ...
    await update.message.reply_text(
        ...
        "ساعتی که میخوای هر روز بهت یادآوری بشه رو بنویس.\n"
...
async def set_reminder(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        await update.message.reply_text(
            f"✅ یادآوری روزانه ساعت *{hour}:00* تنظیم شد! ⏰",
            parse_mode="Markdown"
        )
    except ValueError:
